...
exports.dedupPoints = exports.cropToMargins = exports.scaleToPaper = exports.formatDuration = void 0;
...
function scaleToFit(pointLists, targetMin, targetMax) {
    ...
    const targetCenter = vec_1.vadd(targetMin, vec_1.vmul(vec_1.vsub(targetMax, targetMin), 0.5));
    const offset = vec_1.vsub(targetCenter, vec_1.vmul(vec_1.vsub(max, min), scale * 0.5));
    return pointLists.map((pl) => pl.map((p) => vec_1.vadd(vec_1.vmul(vec_1.vsub(p, min), scale), offset)));
}
/** Scale a drawing to fill a piece of paper, with the given size and margins. */
function scaleToPaper(pointLists, paperSize, marginMm) {
    return scaleToFit(pointLists, { x: marginMm, y: marginMm }, vec_1.vsub(paperSize.size, { x: marginMm, y: marginMm }));
}
exports.scaleToPaper = scaleToPaper;
/**
...
function liangBarsky(aabb, seg) {
    ...
    if (u1 > u2 || u1 > 1 || u1 < 0)
        return null;
    return vec_1.vadd(a, vec_1.vmul(delta, u1));
}
/**
 ...
 */
function cropToMargins(pointLists, paperSize, marginMm) {
    const pageAabb = [{ x: 0, y: 0 }, paperSize.size];
    const margin = { x: marginMm, y: marginMm };
    const insetAabb = [vec_1.vadd(pageAabb[0], margin), vec_1.vsub(pageAabb[1], margin)];
    const truncatedPointLists = [];
...
exports.cropToMargins = cropToMargins;
function dedupPoints(points, epsilon) {
    ...
    for (const p of points.slice(1)) {
        if (vec_1.vlen2(vec_1.vsub(p, dedupedPoints[dedupedPoints.length - 1])) > epsilon2) {
            dedupedPoints.push(p);
        }
    }
    return dedupedPoints;
}
exports.dedupPoints = dedupPoints;
